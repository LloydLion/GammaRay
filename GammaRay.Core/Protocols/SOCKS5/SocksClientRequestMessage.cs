...
public readonly struct SocksClientRequestMessage(SocksClientCommand command, SocksAddressType addressType, ReadOnlyMemory<byte> address, int port)
{
	public SocksClientCommand Command { get; } = command;
	...
	public static async ValueTask<SocksClientRequestMessage> ReadMessageFromSocketAsync(Socket socket, Memory<byte> internalMessageBuffer)
	{
		await socket.ReceiveExactAsync(internalMessageBuffer[..4]);

		var command = (SocksClientCommand)internalMessageBuffer.Span[1];
		...
		return new SocksClientRequestMessage(command, addressType, internalMessageBuffer[..addressLength], port);
	}
}
